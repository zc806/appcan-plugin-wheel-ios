...
#define kFirstImageOffset 6   // degrees
#define kImageSeparation  18    // degrees
#import "MyButton.h"
...
-(NSString*)addPrefix:(NSString*)fileName{
    NSString * string=[NSString stringWithFormat:@"uexRouletteMenu/%@",fileName];
    return string;
}
-(id)initWithFrame:(CGRect)frame buttonArray:(NSArray*)buttonArray leftHanded:(BOOL)leftHanded arcCenter:(CGPoint)theCenter radius:(double)theRadius background:(NSString*)image{
    ...
    if (self) {
        ...
        self.controlView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
        
        UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(80, frame.size.height-240, 240, 240)];
        imageView.image = [UIImage imageWithContentsOfFile:image];
        [self.controlView addSubview:imageView];
        [imageView release];
        //    self.controlView = [[UIView alloc] initWithFrame:CGRectMake(50, 300, 270, 268)];
...
-(void)positionImages {
    for (int i=0;i<[buttons count];i++) {
        ...
        MyButton *btnView = [buttons objectAtIndex:actualIndex];
        double x,y,a;
        double offset=0;
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    //    NSLog(@"this is  touch began 111111111111111");
...
-(void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    //    NSLog(@"this is  touch move  000000000000000000000");
